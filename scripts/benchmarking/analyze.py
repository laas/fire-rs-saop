...
def main(args):
    ...
    for subfolder in list_scenario_subfolders:
        ...
        result_files[fold] = [f.path for f in os.scandir(subfolder) if os.path.splitext(f)[1] == '.json']
        result_files[fold].sort()

    # array that will be used to store one dictionnary per (benchmark-instance, vns-configuration) instance
    all_lines = []

    for run_id, run_json_files in result_files.items():
        print("Benchmark : " + str(run_id))
        ...
        for conf in df["configuration_name"].unique():
            ...
            normalized_utility_histories = [hist / u_star for (_, (hist, u_star))
                                            in
                                            df_of_conf[['utility_history', 'utility_star']].iterrows()]
